...
export const BUNDLES: Record<Exclude<BundleId, "custom">, BundleDefinition> = {
  ...
  full: { id: "full", label: "Full", description: "All bundled integrations; high-risk capabilities remain disabled until configured and authorized.", features: ["durable-harness", "filesystem", "shell", "git", "web", "memory", "skills", "delegation", "workspaces", "mcp-client", "mcp-importer", "mcp-server", "scheduler", "browser", "channels", "execution-nodes", "webui"], pluginDefaults: [] },
};

const FEATURE_CAPABILITIES: Record<string, string[]> = {
  filesystem: ["filesystem.read", "filesystem.write"], shell: ["process"], web: ["network"], "mcp-client": ["plugin", "process"], "mcp-server": ["network.listen"], scheduler: ["scheduler.execute"], browser: ["browser.control", "network"], channels: ["network", "messaging.send"], "execution-nodes": ["process.remote"], delegation: ["agent.delegate"], memory: ["memory.read", "memory.write"],
};

export function featureEnabled(config: CorvusConfig, feature: string): boolean { return (config.installation?.features ?? BUNDLES.default.features).includes(feature); }

export function currentBundle(config: CorvusConfig): BundleId { return config.installation?.bundle ?? "default"; }
export function planBundle(config: CorvusConfig, target: Exclude<BundleId, "custom">): BundlePlan {
  const definition = BUNDLES[target]; const enabled = new Set(config.installation?.features ?? BUNDLES[currentBundle(config) === "custom" ? "default" : currentBundle(config) as Exclude<BundleId,"custom">].features);
...
export function applyPermissionPreset(config: CorvusConfig, preset: Exclude<PermissionPreset, "custom">): void {
  ...
  config.installation = { ...(config.installation ?? { bundle: "default", features: [...BUNDLES.default.features] }), permissionPreset: preset, updatedAt: new Date().toISOString() };
}
