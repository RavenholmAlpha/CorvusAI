  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (requestUrl.pathname === "/api/v1/runtime/capabilities" && req.method === "GET") {
        send(res, 200, { serverVersion: "0.2.0", bundle: options.config.installation, features: options.config.installation?.features ?? [], pages: capabilityPages(options.config.installation?.features ?? []) }); return;
      }
      ...
      if (sessionContext && req.method === "GET") {
        ...
        if (!contextUsage) {
          ...
          const estTokens = sysTokens + userTokens + asstTokens;
          const windowTokens = options.config.contextWindowTokens ?? 128000;
          contextUsage = {
            messageCount: messages.length,
            estimatedTokens: estTokens,
            lastRequestTokens: estTokens,
            memoryBreakdown: { system: sysTokens, user: userTokens, assistant: asstTokens, tool: 0 },
            lastRequestBreakdown: { system: sysTokens, user: userTokens, assistant: asstTokens, tool: 0 },
