  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (requestUrl.pathname === "/api/v1/runtime/capabilities" && req.method === "GET") {
        send(res, 200, { serverVersion: "0.2.2", bundle: options.config.installation, features: options.config.installation?.features ?? [], pages: capabilityPages(options.config.installation?.features ?? []) }); return;
      }
