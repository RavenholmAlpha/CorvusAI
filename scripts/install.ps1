...
if(-not $Preset){if($NonInteractive){$Preset="default"}else{$Preset=Select-Menu "Feature bundle" @(@{Label="Minimal";Value="minimal";Description="Core, filesystem, shell and WebUI."},@{Label="Default";Value="default";Description="Recommended developer setup with memory, skills, delegation and MCP."},@{Label="Full";Value="full";Description="Adds browser, scheduler, channels, execution nodes and MCP server."}) 2}}
